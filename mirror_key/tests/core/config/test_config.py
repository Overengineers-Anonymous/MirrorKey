...
from mirror_key.core.config.config import ConfigParser
from mirror_key.core.config.structure import Config, ApiDescriptor
from mirror_key.tests import parametrise, Param


def test_load_config_file():
    # with patch.object(ConfigParser._validate_config) as mock:
    with patch("mirror_key.core.config.config.ConfigParser._validate_config") as mock:
        config = ConfigParser()
...
def test_invalid_config():
    with patch("mirror_key.core.config.config.ConfigParser.parse_errors") as mock:
        config = ConfigParser()
...
def test_api_descriptor_parser_error(cfg_data: Config, expected: str):
    with patch("mirror_key.core.config.config.logger.error") as mock:
        config = ConfigParser()
...
def test_api_descriptor_parser_required_missing_config_invalid(required: bool):
    with patch("mirror_key.core.config.config.ConfigParser.parse_errors", wraps=mock_exit) as mock:
        config = ConfigParser()
